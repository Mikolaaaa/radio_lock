...
def handle_udp_data(data, client_address):
        ...
        if param1 == 2 or (param1 == 1 and param2 == 1):
            ...
            if param1 == 2:
                if param5 == 0 and param2 < 4 and param3 < 3 and (param4 < 2 or param4 == 240):
                    lvl_0 = execute_query(f"select level_1 from level {param3_new}")
                    for item in lvl_0:
                        for num in item:
                            if num > param_lvl >= 0:
                    ...
                    lvl_0 = execute_query(f"select level_2 from level {param3_new}")
                    lvl_1 = execute_query(f"select level_0 from level {param3_new}")
                    for item in lvl_1:
                        for num1 in item:
                            lvl_1 = num1
                    for item in lvl_0:
    ...
    if param1 == 1 and param2 == 0 and param3 < 3 and param5 < 4:
        if param4 == 240:
            if param6_new != '':
                ...
                result = execute_query(f"{param5_new} {param1_new} {param3_new} order by time desc limit 1")
        elif param4 == 0 or param4 == 1:
            if param6_new != '':
                result = execute_query(
                    f"{param5_new} {param1_new} {param3_new} {param6_new} order by time desc limit 1")
        ...
        result = execute_query(f"{param2_new}")
        result += execute_query(f"{param3_new}")
        result += execute_query(f"{param4_new}")
        result += execute_query(f"{param5_new}")
        ...
        if param2_new != param_lvl:
            if param3_new != '' and param2_new != '' and param1_new != '' and param4_new != '' and param5_new != '' and param6_new != '':
                execute_query(f"{param2_new} {param3_new}")
                result = execute_query(f"{param1_new} {param3_new}")
                result += execute_query(f"{param5_new} {param4_new} {param6_new} order by time desc limit 1")
        else:
            result = param_lvl
    elif param1 == 1 and param2 == 1:
        if param3_new != '' and param2_new != '' and param1_new != '' and param4_new != '' and param5_new != '' and param6_new != '':
            result = execute_query(f"{param1_new} {param2_new} {param3_new}")
            result += execute_query(f"{param5_new} {param4_new} {param6_new} order by time desc limit 1")
    ...
    elif (len(data) == 8) and ((param1 > 2) or (param2 > 3) or (param3 > 2) or (param4 > 1 and param4 != 240) or (param5 > 4)) or result == []:
        get_data = send_udp_data('error', param1, param2, param3, param4, param5)
    elif (len(data) > 9) and ((param1 > 2) or (param2 > 3) or (param3 > 2) or (param4 > 1 and param4 != 240) or (param5 > 4)) or result == []:
        get_data = send_udp_data(number, param1, param2, param3, param4, param5)
    elif (len(data) == 9) and ((param1 > 2) or (param2 > 3) or (param3 > 2) or (param4 > 1 and param4 != 240) or (param5 > 4)) or result == []:
...
def start_udp_server(host, port):
    ...
    atexit.register(lambda: exit_handler(host_client, port_client, udp_server_socket))
...
def exit_handler(host, port, udp_server_socket):

    print("Сервер завершает работу. Отправляем информацию о завершении.")
    # Отправка сообщения о завершении клиенту
    message = b'@\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
    udp_server_socket.sendto(message, (host, port))
    # Закрытие сокета
